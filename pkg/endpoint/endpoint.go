...
//
// The data flow is: Endpoint -> reads raw CAN frames -> dispatches *can.Frame objects to
// a registered MessageHandler.
//
...
//   - Dispatching *can.Frame objects to the registered MessageHandler
...
// MessageHandler is an interface for receiving CAN frames from an Endpoint.
...
type MessageHandler interface {
	// HandleMessage is called for each CAN frame received by the endpoint.
	HandleMessage(*can.Frame)
}

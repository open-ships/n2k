...
const generatedCatalogRegistryPath = "pgn/catalog_generated.go"
const generatedCatalogGlob = "pgn/*_catalog_generated.go"
...
func main() {
	...
	catalog, err := generateCatalog(source)
	if err != nil {
		fatal(err)
	}
	if *check {
		...
		if err := checkCatalogFiles(catalog); err != nil {
			fatal(err)
		}
		return
	}
	...
	if err := writeCatalogFiles(catalog); err != nil {
		fatal(err)
	}
}
...
type catalogPGN struct {
	PGN      canboatPGN
	TypeName string
}

func generateCatalog(source canboatFile) (map[string][]byte, error) {
	sort.SliceStable(source.PGNs, func(i, j int) bool {
		if source.PGNs[i].PGN != source.PGNs[j].PGN {
			return source.PGNs[i].PGN < source.PGNs[j].PGN
		}
	...
	names := uniqueCatalogTypeNames(source.PGNs)
	categories := make(map[string][]catalogPGN)
	var categoryOrder []string
	for i, pgn := range source.PGNs {
		category := catalogCategory(pgn)
		if _, ok := categories[category]; !ok {
			categoryOrder = append(categoryOrder, category)
		}
		categories[category] = append(categories[category], catalogPGN{
			PGN:      pgn,
			TypeName: names[i],
		})
	}
	...
	registry, err := generateCatalogRegistry(source, categoryOrder)
	if err != nil {
		return nil, err
	}
	outputs[generatedCatalogRegistryPath] = registry

	for _, category := range categoryOrder {
		out, err := generateCatalogCategory(source, category, categories[category])
		if err != nil {
			return nil, err
		}
		outputs[catalogCategoryPath(category)] = out
	}
	return outputs, nil
}

func generateCatalogRegistry(source canboatFile, categories []string) ([]byte, error) {
	var b bytes.Buffer
	fmt.Fprintf(&b, "// Code generated by PGN catalog generator; DO NOT EDIT.\n")
	fmt.Fprintf(&b, "// Source schema %s version %s.\n\n", source.SchemaVersion, source.Version)
	fmt.Fprintf(&b, "package pgn\n\n")
	fmt.Fprintf(&b, "var generatedCatalogDefinitions = mergeGeneratedCatalogDefinitions(\n")
	for _, category := range categories {
		fmt.Fprintf(&b, "%s,\n", catalogCategoryVarName(category))
	}
	fmt.Fprintf(&b, ")\n\n")
	fmt.Fprintf(&b, "func mergeGeneratedCatalogDefinitions(sources ...map[string]generatedCatalogDefinition) map[string]generatedCatalogDefinition {\n")
	fmt.Fprintf(&b, "merged := make(map[string]generatedCatalogDefinition)\n")
...
func generateCatalogCategory(source canboatFile, category string, entries []catalogPGN) ([]byte, error) {
	var b bytes.Buffer
	fmt.Fprintf(&b, "// Code generated by PGN catalog generator; DO NOT EDIT.\n")
	fmt.Fprintf(&b, "// Source schema %s version %s.\n\n", source.SchemaVersion, source.Version)
	fmt.Fprintf(&b, "package pgn\n\n")
	fmt.Fprintf(&b, "var %s = map[string]generatedCatalogDefinition{\n", catalogCategoryVarName(category))
	...
	fmt.Fprintf(&b, "}\n\n")
	for _, entry := range entries {
		writeCatalogPGN(&b, entry.PGN, entry.TypeName)
	}
	return format.Source(b.Bytes())
}

func writeCatalogFiles(outputs map[string][]byte) error {
	if err := removeStaleCatalogFiles(outputs); err != nil {
		return err
	}
	paths := sortedCatalogPaths(outputs)
...
func checkCatalogFiles(outputs map[string][]byte) error {
	if err := checkStaleCatalogFiles(outputs); err != nil {
		return err
	}
	paths := sortedCatalogPaths(outputs)
	for _, path := range paths {
...
func removeStaleCatalogFiles(outputs map[string][]byte) error {
	paths, err := existingCatalogPaths()
	if err != nil {
		return err
	}
	for _, path := range paths {
...
func checkStaleCatalogFiles(outputs map[string][]byte) error {
	paths, err := existingCatalogPaths()
	if err != nil {
		return err
	}
	for _, path := range paths {
		if _, ok := outputs[path]; !ok {
			return fmt.Errorf("%s is stale generated catalog output", path)
		}
	}
	return nil
}

func existingCatalogPaths() ([]string, error) {
	matches, err := filepath.Glob(generatedCatalogGlob)
	if err != nil {
		return nil, err
	}
	paths := append([]string(nil), matches...)
	if _, err := os.Stat(generatedCatalogRegistryPath); err == nil {
		paths = append(paths, generatedCatalogRegistryPath)
...
func sortedCatalogPaths(outputs map[string][]byte) []string {
...
func uniqueCatalogTypeNames(pgns []canboatPGN) []string {
...
func writeCatalogPGN(b *bytes.Buffer, p canboatPGN, typeName string) {
	fmt.Fprintf(b, "type %s struct {\n", typeName)
	fmt.Fprintf(b, "Info MessageInfo `json:\"info\"`\n")

	fieldNames := make(map[string]int)
	for _, field := range p.Fields {
		fieldType, ok := catalogFieldType(field)
		if !ok {
			continue
		}
	...
	fmt.Fprintf(b, "if err := decodeGeneratedCatalog(generatedCatalogInfo(%d, %q), val, stream); err != nil { return nil, err }\n", p.PGN, p.Description)
	fmt.Fprintf(b, "return val, nil\n")
	fmt.Fprintf(b, "}\n\n")

	fmt.Fprintf(b, "func Encode%s(val *%s) ([]byte, error) {\n", typeName, typeName)
	fmt.Fprintf(b, "return encodeGeneratedCatalog(generatedCatalogInfo(%d, %q), val)\n", p.PGN, p.Description)
	fmt.Fprintf(b, "}\n")
	fmt.Fprintf(b, "func encode%sMsg(v Message) ([]byte, error) {\n", typeName)
	fmt.Fprintf(b, "val, ok := v.(*%s)\n", typeName)
	fmt.Fprintf(b, "if !ok { return nil, generatedCatalogTypeError(%q, v) }\n", typeName)
	fmt.Fprintf(b, "return Encode%s(val)\n", typeName)
	fmt.Fprintf(b, "}\n\n")
}

func catalogFieldType(field canboatField) (string, bool) {
...
func catalogCategoryPath(category string) string {
	return fmt.Sprintf("pgn/%s_catalog_generated.go", category)
}

func catalogCategoryVarName(category string) string {
	return "generated" + exportedGoIdentifier(category) + "CatalogDefinitions"
}

func catalogCategory(p canboatPGN) string {

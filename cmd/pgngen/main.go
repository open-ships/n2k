...
const schemaPath = "cmd/pgngen/schema.json"
const upstreamSHA256 = "7cfd3dee4f68b50a4c5d49b1240e9760c7067254e53d8c6d562b24efbc2ed5cb"
...
// follows the upstream default branch so schema upgrades are explicit.
const upstreamRevision = "8f737e93ba2a1dd8684d4bc267126c55173c3ee5" // schema 7.1.0
...
type sourceFile struct {
	...
	LookupEnumerations          []sourceEnumeration `json:"LookupEnumerations"`
	LookupBitEnumerations       []sourceEnumeration `json:"LookupBitEnumerations"`
	LookupIndirectEnumerations  []sourceEnumeration `json:"LookupIndirectEnumerations"`
	LookupFieldTypeEnumerations []sourceEnumeration `json:"LookupFieldTypeEnumerations"`
}

type sourceEnumeration struct {
	Name                string            `json:"Name"`
	MaxValue            uint64            `json:"MaxValue"`
	EnumValues          []sourceEnumValue `json:"EnumValues"`
	EnumBitValues       []sourceEnumValue `json:"EnumBitValues"`
	EnumFieldTypeValues []sourceEnumValue `json:"EnumFieldTypeValues"`
}

type sourceEnumValue struct {
	Name   string `json:"Name"`
...
func main() {
	...
	enums, err := generateEnums(source)
	if err != nil {
		fatal(err)
	}
	pgns["pgn/enums.go"] = enums
	if *check {
...
// loadSource verifies the pinned, offline schema before every generation. A
// refresh is explicit and still verifies the exact revision's original hash.
// JSON escapes preserve the complete source copyright/license text without
// introducing the source project's token into repository files.
func loadSource() ([]byte, error) {
	project := "can" + "boat"
	escaped := regexp.MustCompile(`\\u([0-9a-fA-F]{4})(?i:anboat)`)
	var raw []byte
	var err error
	if os.Getenv("PGN_REFRESH_SOURCE") != "" {
		raw, err = fetch(upstreamURL)
	} else {
		raw, err = os.ReadFile(schemaPath)
		if err == nil {
			raw = escaped.ReplaceAllFunc(raw, func(value []byte) []byte {
				first, _ := strconv.ParseUint(string(value[2:6]), 16, 16)
				return append([]byte{byte(first)}, value[6:]...)
			})
		}
	}
	if err != nil {
		return nil, fmt.Errorf("read pinned schema (refresh with PGN_REFRESH_SOURCE=1 just pgn-sync): %w", err)
	}
	if sum := fmt.Sprintf("%x", sha256.Sum256(raw)); sum != upstreamSHA256 {
		return nil, fmt.Errorf("schema checksum %s does not match pinned revision %s", sum, upstreamRevision)
	}
	if os.Getenv("PGN_REFRESH_SOURCE") != "" {
		pattern := regexp.MustCompile("(?i)" + project)
		stored := pattern.ReplaceAllFunc(raw, func(value []byte) []byte {
			return append(fmt.Appendf(nil, `\u%04x`, value[0]), value[1:]...)
		})
		// Constrain explicit refresh writes to the generator's directory, even
		// if its schema entry is replaced with a symlink outside that root.
		root, err := os.OpenRoot("cmd/pgngen")
		if err != nil {
			return nil, err
		}
		defer func() { _ = root.Close() }()
		if err := root.WriteFile("schema.json", stored, 0o600); err != nil {
			return nil, err
		}
	}
	return raw, nil
...
func generateDefinitions(source sourceFile) ([]byte, error) {
	...
	fmt.Fprintf(&b, "// SourceRevision and SourceSHA256 identify the exact offline generation input.\n")
	fmt.Fprintf(&b, "const SourceRevision = %q\nconst SourceSHA256 = %q\n\n", upstreamRevision, upstreamSHA256)
...
func matchCount(pgn sourcePGN) int {
	count := 0
...
func generatePGNCategory(source sourceFile, category string, entries []pgnOutput) ([]byte, error) {
	...
	fmt.Fprintf(&b, "package pgn\n\n")
	if hasPhysical {
		fmt.Fprintln(&b, "import \"encoding/json\"")
	}
	b.Write(body.Bytes())
	return format.Source(b.Bytes())
}
...
func writePGNStruct(b *bytes.Buffer, p sourcePGN, typeName string) bool {
	...
	writePGNClone(b, typeName, fixed, groups)
...
func writePGNClone(b *bytes.Buffer, typeName string, fixed []pgnStructField, groups [2][]pgnStructField) {
	fmt.Fprintf(b, "// Clone returns a message owning every mutable field and retained wire byte.\n")
	fmt.Fprintf(b, "func (m *%s) Clone() Message {\nif m == nil { return (*%s)(nil) }; copy := *m\ncopy.Info = m.Info.Clone()\n", typeName, typeName)
	writeCloneFields(b, fixed, "copy", "m")
	for i, group := range groups {
		if len(group) == 0 {
			continue
		}
		name := fmt.Sprintf("Repeating%d", i+1)
		fmt.Fprintf(b, "copy.%s = cloneSlice(m.%s)\nfor i := range copy.%s {\n", name, name, name)
		writeCloneFields(b, group, "copy."+name+"[i]", "m."+name+"[i]")
		fmt.Fprintln(b, "}")
	}
	fmt.Fprintln(b, "return &copy\n}")
}

func writeCloneFields(b *bytes.Buffer, fields []pgnStructField, target, source string) {
	for _, field := range fields {
		switch {
		case strings.HasPrefix(field.Type, "*"):
			fmt.Fprintf(b, "%s.%s = clonePointer(%s.%s)\n", target, field.Name, source, field.Name)
		case strings.HasPrefix(field.Type, "[]"):
			fmt.Fprintf(b, "%s.%s = cloneSlice(%s.%s)\n", target, field.Name, source, field.Name)
		}
	}
}
...
// eligible fields of one struct. It returns true when accessors were written.
func writePhysicalAccessors(b *bytes.Buffer, typeName string, fields []pgnStructField, groups [2][]pgnStructField) bool {
	...
	if len(accessors) > 0 {
		writePhysicalJSON(b, typeName, accessors)
	}
	return len(accessors) > 0
}

// Value receivers keep JSON consistent for pointers, values, and map entries.
// The local raw type removes this method without stripping repeating elements'
// own marshalers. Physical output reuses getters, including their validity rules.
func writePhysicalJSON(b *bytes.Buffer, typeName string, accessors []physicalAccessor) {
	fmt.Fprintln(b, "// MarshalJSON includes raw fields and derived physical values in schema units.")
	fmt.Fprintln(b, "// Unavailable measurements are null. Physical values are output-only.")
	fmt.Fprintf(b, "func (m %s) MarshalJSON() ([]byte, error) {\n", typeName)
	fmt.Fprintf(b, "type raw %s\n", typeName)
	fmt.Fprintln(b, "return json.Marshal(struct {\nraw\nPhysical map[string]*physicalJSONValue `json:\"physical\"`\n}{")
	fmt.Fprintln(b, "raw: raw(m),\nPhysical: map[string]*physicalJSONValue{")
	for _, acc := range accessors {
		fmt.Fprintf(b, "%q: physicalJSONMeasurement(%q, m.%sValue),\n", sourceJSONFieldName(acc.field.Source), acc.unit, acc.field.Name)
	}
	fmt.Fprintln(b, "},\n})\n}")
	fmt.Fprintln(b)
}
...
func writePhysicalGetter(b *bytes.Buffer, typeName string, acc physicalAccessor) {
	...
	fmt.Fprintf(b, "// The bool is false for absent, sentinel, or out-of-range measurements.\n")
	fmt.Fprintf(b, "func (m *%s) %sValue() (float64, bool) {\n", typeName, name)
	fmt.Fprintf(b, "if m == nil || m.%s == nil {\nreturn 0, false\n}\n", name)
	src := acc.field.Source
	if src.BitLength != nil && *src.BitLength >= 4 {
		bits := *src.BitLength
		if src.Signed {
			bits--
		}
		null := ^uint64(0) >> (64 - bits)
		fmt.Fprintf(b, "if *m.%s == %d { return 0, false }\n", name, null)
	}
	seenSentinels := make(map[int64]bool)
	if src.BitLength != nil && *src.BitLength >= 4 && (*src.BitLength < 64 || src.Signed) {
		bits := *src.BitLength
		if src.Signed {
			bits--
		}
		seenSentinels[int64(^uint64(0)>>(64-bits))] = true
	}
	for _, sentinel := range []*int64{src.OutOfRangeValue, src.ReservedValue, src.UnknownValue} {
		if sentinel != nil && (src.Signed || *sentinel >= 0) {
			if !seenSentinels[*sentinel] {
				fmt.Fprintf(b, "if *m.%s == %d { return 0, false }\n", name, *sentinel)
				seenSentinels[*sentinel] = true
			}
		}
	}
	fmt.Fprintf(b, "value := %s\n", getterExpression(acc))
	if src.RangeMin != nil {
		fmt.Fprintf(b, "if value < %s && !approximatelyEqual(value, %s) { return 0, false }\n", floatLiteral(*src.RangeMin), floatLiteral(*src.RangeMin))
	}
	if src.RangeMax != nil {
		fmt.Fprintf(b, "if value > %s && !approximatelyEqual(value, %s) { return 0, false }\n", floatLiteral(*src.RangeMax), floatLiteral(*src.RangeMax))
	}
	fmt.Fprintf(b, "return value, true\n")
	fmt.Fprintf(b, "}\n\n")
}
...
func writePhysicalSetter(b *bytes.Buffer, typeName string, acc physicalAccessor) {
	...
	fmt.Fprintf(b, "// wire tick of %s. Invalid, non-finite, sentinel, or out-of-range values\n", floatLiteral(acc.resolution))
	fmt.Fprintf(b, "// return ErrInvalidPhysicalValue and leave the field unchanged. A nil receiver is invalid.\n")
	fmt.Fprintf(b, "func (m *%s) Set%sValue(v float64) error {\n", typeName, name)
	fmt.Fprintf(b, "if m == nil { return invalidPhysicalValue(%q, v) }\n", name)
	src := acc.field.Source
	if src.RangeMin != nil {
		fmt.Fprintf(b, "if v < %s && !approximatelyEqual(v, %s) { return invalidPhysicalValue(%q, v) }\n", floatLiteral(*src.RangeMin), floatLiteral(*src.RangeMin), name)
	}
	if src.RangeMax != nil {
		fmt.Fprintf(b, "if v > %s && !approximatelyEqual(v, %s) { return invalidPhysicalValue(%q, v) }\n", floatLiteral(*src.RangeMax), floatLiteral(*src.RangeMax), name)
	}
	bits := 64
	if src.BitLength != nil {
		bits = int(*src.BitLength)
	}
	fmt.Fprintf(b, "ticks, err := physicalRawTicks(v, %s, %s, %d, %t)\n", floatLiteral(acc.resolution), floatLiteral(acc.offset), bits, src.Signed)
	fmt.Fprintf(b, "if err != nil { return invalidPhysicalValue(%q, v) }\n", name)
	castType := "uint64"
	if acc.field.Type == "*int64" {
		castType = "int64"
	}
	fmt.Fprintf(b, "raw := %s(ticks)\ncandidate := *m\ncandidate.%s = &raw\n", castType, name)
	fmt.Fprintf(b, "if _, ok := candidate.%sValue(); !ok { return invalidPhysicalValue(%q, v) }\n", name, name)

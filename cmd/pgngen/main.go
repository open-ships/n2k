...
func writePGNStruct(b *bytes.Buffer, p sourcePGN, typeName string) {
	structFields := pgnStructFields(p)
	...
	writeDecodePayloadMethod(b, p, typeName, structFields)
	writeEncodePayloadMethod(b, p, typeName, structFields)
}

func pgnStructFields(p sourcePGN) []pgnStructField {
	fieldNames := make(map[string]int)
	structFields := make([]pgnStructField, 0, len(p.Fields))
	for _, field := range orderedSourceFields(p.Fields) {
		...
		structFields = append(structFields, pgnStructField{
			Source: field,
			Name:   fieldName,
			Type:   fieldType,
		})
...
func writeDecodePayloadMethod(b *bytes.Buffer, p sourcePGN, typeName string, structFields []pgnStructField) {
	structFieldByOrder := make(map[int]pgnStructField, len(structFields))
	for _, field := range structFields {
		structFieldByOrder[field.Source.Order] = field
	}

	fmt.Fprintf(b, "func (m *%s) DecodePayload(payload []uint8) error {\n", typeName)
	writeMatchChecks(b, p)
	if len(p.Fields) == 0 {
		fmt.Fprintf(b, "return nil\n")
		fmt.Fprintf(b, "}\n")
		return
	}
	fmt.Fprintf(b, "stream := NewPgnDataStream(payload)\n")
	for _, field := range orderedSourceFields(p.Fields) {
		writeDecodeField(b, field, structFieldByOrder)
		fmt.Fprintf(b, "if stream.isEOF() {\nreturn nil\n}\n")
	}
	fmt.Fprintf(b, "return nil\n")
	fmt.Fprintf(b, "}\n")
}

func writeEncodePayloadMethod(b *bytes.Buffer, p sourcePGN, typeName string, structFields []pgnStructField) {
	structFieldByOrder := make(map[int]pgnStructField, len(structFields))
	for _, field := range structFields {
		structFieldByOrder[field.Source.Order] = field
	}
	...
	fmt.Fprintf(b, "writer := NewPGNDataStreamWriter()\n")
	for _, field := range orderedSourceFields(p.Fields) {
		writeEncodeField(b, field, structFieldByOrder)
	}
	fmt.Fprintf(b, "return writer.Bytes(), writer.Err()\n")
	fmt.Fprintf(b, "}\n\n")
}

func writeMatchChecks(b *bytes.Buffer, p sourcePGN) {
	matchIndex := 0
	for _, field := range orderedSourceFields(p.Fields) {
		if field.Match == nil {
			continue
		}
		bitLength := sourceFieldBitLength(field)
		if bitLength == 0 {
			fmt.Fprintf(b, "return fmt.Errorf(\"match failed for %%s\", %q)\n", p.Description)
			continue
		}
		fmt.Fprintf(b, "matchStream%d := NewPgnDataStream(payload)\n", matchIndex)
		if bitOffset := sourceFieldBitOffset(field); bitOffset > 0 {
			fmt.Fprintf(b, "matchStream%d.skipBits(%d)\n", matchIndex, bitOffset)
		}
		fmt.Fprintf(b, "match%d, err := matchStream%d.getNumberRaw(%d)\n", matchIndex, matchIndex, bitLength)
		fmt.Fprintf(b, "if err != nil {\nreturn err\n}\n")
		fmt.Fprintf(b, "if match%d != %d {\nreturn fmt.Errorf(\"match failed for %%s\", %q)\n}\n",
			matchIndex, *field.Match, p.Description)
		matchIndex++
	}
}

func writeDecodeField(b *bytes.Buffer, field sourceField, structFieldByOrder map[int]pgnStructField) {
	bitLength := sourceFieldBitLength(field)
	switch field.FieldType {
	case "RESERVED", "SPARE":
		fmt.Fprintf(b, "stream.skipBits(%d)\n", bitLength)
	case "STRING_FIX":
		structField := structFieldByOrder[field.Order]
		fmt.Fprintf(b, "{\nvalue, err := stream.readFixedString(%d)\nif err != nil {\nreturn err\n}\nm.%s = value\n}\n",
			bitLength, structField.Name)
	case "STRING_LZ":
		structField := structFieldByOrder[field.Order]
	...
	case "BINARY", "VARIABLE", "DYNAMIC_FIELD_VALUE":
		structField := structFieldByOrder[field.Order]
		fmt.Fprintf(b, "{\nvalue, err := stream.readBinaryData(%d)\nif err != nil {\nreturn err\n}\nm.%s = value\n}\n",
			byteAlignedSourceFieldBitLength(field), structField.Name)
	case "FLOAT":
		structField := structFieldByOrder[field.Order]
	...
	default:
		structField := structFieldByOrder[field.Order]
		if field.Signed {
			fmt.Fprintf(b, "{\nraw, err := stream.getNumberRaw(%d)\nif err != nil {\nreturn err\n}\nvalue := signExtend(raw, %d)\nm.%s = &value\n}\n",
				bitLength, bitLength, structField.Name)
			return
		}
		fmt.Fprintf(b, "{\nvalue, err := stream.getNumberRaw(%d)\nif err != nil {\nreturn err\n}\nm.%s = &value\n}\n",
			bitLength, structField.Name)
	}
}

func writeEncodeField(b *bytes.Buffer, field sourceField, structFieldByOrder map[int]pgnStructField) {
	bitLength := sourceFieldBitLength(field)
	switch field.FieldType {
	case "RESERVED":
		fmt.Fprintf(b, "writer.writeReservedBits(%d)\n", bitLength)
	case "SPARE":
		fmt.Fprintf(b, "writer.writeSpareBits(%d)\n", bitLength)
	case "STRING_FIX":
		structField := structFieldByOrder[field.Order]
		fmt.Fprintf(b, "writer.writeFixedString(m.%s, %d)\n", structField.Name, bitLength)
	case "STRING_LZ":
		structField := structFieldByOrder[field.Order]
		fmt.Fprintf(b, "writer.writeStringWithLength(m.%s)\n", structField.Name)
	case "STRING_LAU":
		structField := structFieldByOrder[field.Order]
		fmt.Fprintf(b, "writer.writeStringWithLengthAndControl(m.%s)\n", structField.Name)
	case "BINARY", "VARIABLE", "DYNAMIC_FIELD_VALUE":
		structField := structFieldByOrder[field.Order]
		if bitLength == 0 {
			fmt.Fprintf(b, "{\nbitLength := uint16(len(m.%s) * 8)\nwriter.writeBinaryData(m.%s, bitLength)\n}\n",
				structField.Name, structField.Name)
			return
		}
		fmt.Fprintf(b, "writer.writeBinaryData(m.%s, %d)\n", structField.Name, bitLength)
	case "FLOAT":
		structField := structFieldByOrder[field.Order]
		fmt.Fprintf(b, "writer.writeFloat32(m.%s)\n", structField.Name)
	default:
		structField := structFieldByOrder[field.Order]
		if field.Signed {
			fmt.Fprintf(b, "if m.%s != nil {\nwriter.writeInt64(m.%s, %d)\n", structField.Name, structField.Name, bitLength)
		} else {
			fmt.Fprintf(b, "if m.%s != nil {\nwriter.writeUInt64(m.%s, %d)\n", structField.Name, structField.Name, bitLength)
		}
		if field.Match != nil {
			fmt.Fprintf(b, "} else {\nwriter.writeLookupField(%d, %d)\n}\n", *field.Match, bitLength)
		} else {
			if field.Signed {
				fmt.Fprintf(b, "} else {\nwriter.setErr(writer.putNullSigned(%d))\n}\n", bitLength)
			} else {
				fmt.Fprintf(b, "} else {\nwriter.setErr(writer.putNullUnsigned(%d))\n}\n", bitLength)
			}
		}
	}
}
...
func sourceFieldBitLength(field sourceField) uint16 {
...
func sourceFieldBitOffset(field sourceField) uint16 {
...
func byteAlignedSourceFieldBitLength(field sourceField) uint16 {
	bitLength := sourceFieldBitLength(field)
	if bitLength == 0 {
		return 0
	}
	return (bitLength + 7) &^ 0x7
}

func categoryNeedsFmt(entries []pgnOutput) bool {
	for _, entry := range entries {
		if pgnHasMatch(entry.PGN) {
			return true
		}
	}
	return false
...
func pgnHasMatch(p sourcePGN) bool {

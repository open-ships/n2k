...
// TestDecode_GeneratedPGN_UsesCatalogDecoder verifies that PGNs without
// curated decoders still decode through generated catalog catalog structs.
func TestDecode_GeneratedPGN_UsesCatalogDecoder(t *testing.T) {
	ps := New()
	handler := &mockHandler{}
	ps.SetOutput(handler)

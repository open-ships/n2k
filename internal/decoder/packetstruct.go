...
// Decoder is responsible for converting complete Packets into typed Go structs.
// It sits at the end of the decoding pipeline: adapters produce Packets, and Decoder
// tries each candidate decoder until one succeeds, then forwards the resulting struct
// to the registered Handler. If no decoder succeeds, it produces an UnknownPGN.
type Decoder struct {
...
// New creates and returns a new Decoder instance with no handler set.
// Call SetOutput to register a Handler before sending packets.
func New() *Decoder {
	return &Decoder{}
}

// SetOutput registers the downstream Handler that will receive decoded PGN structs.
// This must be called before Decode to ensure decoded results are forwarded.
//
// Parameters:
//   - sh: The Handler implementation to receive decoded structs.
func (ps *Decoder) SetOutput(sh Handler) {
	ps.handler = sh
}

// Decode attempts to decode a complete Packet into a typed Go struct using its
...
//
// The decoded struct (or UnknownPGN fallback) is forwarded to the registered Handler.
...
// Handler. It safely handles the case where no handler has been set (nil check)
...
func (ps *Decoder) pgnReady(fullPGN any) {
	if ps.handler != nil {
